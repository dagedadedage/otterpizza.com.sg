  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="font-family:system-ui,sans-serif;max-width:600px;margin:0 auto;background:#FEFBF7">
    ...
    <div style="background:white;border:1px solid #E8D5C4;border-top:none;border-radius:0 0 12px 12px;padding:20px">
      ...
      <table style="width:100%;border-collapse:collapse">
        ...
        ${[...data.items].sort((a, b) => a.sku.localeCompare(b.sku)).map((i, idx) => `<tr style="border-bottom:1px solid #E8D5C4"><td style="padding:8px;font-size:11px;color:#8B7355">${idx + 1}</td><td style="padding:8px;font-size:11px;color:#8B7355;font-family:monospace">${i.sku}</td><td style="padding:8px;font-size:13px;color:#2D1B14">${i.name}</td><td style="text-align:center;padding:8px;font-size:13px;color:#2D1B14">${i.quantity}</td><td style="text-align:right;padding:8px;font-size:13px;color:#2D1B14">${formatPrice(i.totalPrice)}</td></tr>`).join("")}
      </table>
...
export async function sendPendingPaymentReminder(data: OrderEmailData) {
  // Wait 30 seconds, then check if order is still PENDING before sending
  const PAYMENT_WINDOW_MS = 30 * 1000;
